...
uint16 zclApp_event_loop(uint8 task_id, uint16 events) {
    ...
    if (events & APP_MOTION_ON_EVT) {
        ...
        osal_start_timerEx(zclApp_TaskID, APP_MOTION_DELAY_EVT, 20000);
        LREPMaster("START_DELAY\r\n");            
        //report
//        zclApp_DS18B20_MeasuredValue = 100;
...
//        bdb_RepChangedAttrValue(zclApp_ThirdEP.EndPoint, OCCUPANCY, ATTRID_MS_OCCUPANCY_SENSING_CONFIG_OCCUPANCY_SENSOR_TYPE);
//        bdb_RepChangedAttrValue(zclApp_ThirdEP.EndPoint, OCCUPANCY, ATTRID_MS_OCCUPANCY_SENSING_CONFIG_PIR_O_TO_U_DELAY);
/*        
    ...
    if (pReportCmd != NULL) {
        ...
        pReportCmd->attrList[1].attrID = ATTRID_MS_OCCUPANCY_SENSING_CONFIG_OCCUPANCY_SENSOR_TYPE;
        pReportCmd->attrList[1].dataType = ZCL_DATATYPE_UINT8;
        pReportCmd->attrList[1].attrData = (void *)(&zclApp_OccType);

        pReportCmd->attrList[2].attrID = ATTRID_MS_OCCUPANCY_SENSING_CONFIG_PIR_O_TO_U_DELAY;
        pReportCmd->attrList[2].dataType = ZCL_DATATYPE_UINT16;
        pReportCmd->attrList[2].attrData = (void *)(&zclApp_PirOccupiedToUnoccupiedDelay);

        afAddrType_t inderect_DstAddr = {.addrMode = (afAddrMode_t)AddrNotPresent, .endPoint = 0, .addr.shortAddr = 0};
        zcl_SendReportCmd(zclApp_ThirdEP.EndPoint, &inderect_DstAddr, OCCUPANCY, pReportCmd, ZCL_FRAME_CLIENT_SERVER_DIR, TRUE, bdb_getZCLFrameCounter());
    }
...
*/           
    ...
    if (events & APP_MOTION_OFF_EVT) {
        ...
        //report
//        zclApp_DS18B20_MeasuredValue = 0;
//        bdb_RepChangedAttrValue(zclApp_SecondEP.EndPoint, TEMP, ATTRID_MS_TEMPERATURE_MEASURED_VALUE);       
        zclApp_Occupied = 0;
        bdb_RepChangedAttrValue(zclApp_ThirdEP.EndPoint, OCCUPANCY, ATTRID_MS_OCCUPANCY_SENSING_CONFIG_OCCUPANCY);
...
//        MicroWait(50);
        ...
        //report
        if (contact){
          zclApp_SoilHumiditySensor_MeasuredValue = 100;
        } else {
...
//          MicroWait(50);
          if (power == 0){
            if (!contact) {
              osal_start_timerEx(zclApp_TaskID, APP_MOTION_OFF_EVT, 1);
            } else {
              osal_start_timerEx(zclApp_TaskID, APP_MOTION_ON_EVT, 1);
            }
          } else {
            power = power - 1;
          }
      ...
      LREP("zclApp_PirOccupiedToUnoccupiedDelay=%d\r\n", zclApp_Config.PirOccupiedToUnoccupiedDelay);
